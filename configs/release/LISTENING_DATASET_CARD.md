...
三组盲听材料，加上四个人填回来的评分和答案表。这是 mindsurf-omni 唯一一把人耳尺子——
其余的音质数字都出自自动指标，而自动指标有它自己的盲点。
...
**`listening_emotion` 否掉了一个训练计划。** 人对 `emotion2vec` 自动标签的一致率是 31–42%，
而人对人是 41–65%。两个数都低说明这个任务本来就难；但人彼此对得上、却都对不上标签，
那是标签系统性偏离人耳。我们原本要拿这批标签做一轮情绪条件化重训，
这个结果说明前提不成立，那一轮没有开。

**`listening_models` 给出了那把人耳尺子。** 我们的成品和上游官方发布权重都是 2.738，
到小数点后三位相同，缺陷标记 43 对 42。在这之前「与官方无法区分」只有 UTMOS
一把自动尺子撑着，现在有一把不可能继承 UTMOS 盲点的仪器给出了同一个判定。
同一个包里的参照上限 edge-tts 是 4.023。
