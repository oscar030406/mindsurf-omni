...
| 问题 | 修法与效果 |
| --- | --- |
| `Conversation` 记账、裁剪、上报 `dropped_turns`，但 `cascade.respond` 只用当前这句话建 prompt，历史从不进模型 | 级联把历史拼进 messages，转写随第一块 chunk 回传给调用方；原生仍是单轮，[原因在指南](docs/INTEGRATION.md)。表现上，「那明天呢」现在能接上上一轮 |
| 识别前不判有没有人声，静音照样送进 SenseVoice，而它对空输入不返回空、会编 | 整段 RMS 低于 0.002 直接返回空转写。实测：三秒静音从「韩文转写 + 9.6 秒英文回复」变成 `asr_empty` |
...
| `temperature` 默认值 0.4 是上游的 0.7 沿用来的，从没测过 | 12 题 × 3 遍量一致率：0.7 是 0.270、0.4 是 0.418、0.2 是 0.479 但回复变长且退化变多，取 0.4。[口径与限制](docs/INTEGRATION.md) |
| 契约漏了已实现且写进文档的 `session.clear`，又声明了三个从来没有发出过的事件 | 补上前者、删掉后者，并加一条测试断言每个声明的事件在服务里都有发出点 |
