...
| temperature | 三遍之间的一致率 | 回复中位字数 | 同题字数极差 | 退化性重复 |
| ---: | ---: | ---: | ---: | ---: |
| 0.7 | 0.270 ± 0.107 | 60 | 38 | 0/36 |
| **0.4** | **0.418 ± 0.165** | 63 | 30 | 1/36 |
| 0.2 | 0.479 ± 0.183 | 76 | 36 | 2/36 |

0.7 到 0.4 这一步一致率涨 0.148，约三到四个标准误；0.4 再往下到 0.2 只涨 0.061，
落在波动里，而且回复变长、退化样本变多。所以停在 0.4。

**这只量了稳定性，没量好坏。** 回答是不是更对、更有用，要判官模型来判，那一轮没跑。
`top_p 0.9` 和 `max_tokens 512` 仍然是上游默认值，没测过。
你们要调就调，调完把前后效果测出来告诉我们。
