...
| 轴 | 参数 | 值 | 状况 |
| --- | --- | ---: | --- |
| 文本 | `temperature` | 0.4 | 可调（契约默认，`contract.py`） |
| | `top_p` | 0.9 | 可调 |
| | `max_tokens` | 512 | 可调 |
...
### 7.4 文本的 temperature 量过稳定性，没量过好坏

默认值从上游的 0.7 换成了 **0.4**，依据是 2026-08-10 的一次对照：12 个问题各问三遍，
量同一问题三遍回答之间的字符相似度（1.0 表示每次一模一样）。

| temperature | 三遍之间的一致率 | 回复中位字数 | 退化性重复 |
| ---: | ---: | ---: | ---: |
| 0.0（贪心） | 1.000 | 68 | 3/36 |
| 0.1 | 0.630 ± 0.205 | 80 | 2/36 |
| 0.2 | 0.479 ± 0.183 | 76 | 2/36 |
| **0.4** | **0.418 ± 0.165** | 63 | 1/36 |
| 0.7（上游默认） | 0.270 ± 0.107 | 60 | 0/36 |

这条轴是有代价的：温度越低越稳定，但退化性重复越多、回复越长。
0.0 的一致率是 1.000（贪心解码必然如此），代价是 36 条里有 3 条陷入复读。
0.7 到 0.4 这一步一致率涨 0.148、约三到四个标准误，而退化只多 1 条，所以停在 0.4——
它是这条曲线上稳定性与复读之间比较划算的一点，不是最稳的那一点。

要更稳可以自己传 `temperature`，服务不拦。真要用 0.0，先想好怎么处理那 8% 的复读。

**两个限制**：一、这只量了稳定性，没量好坏——回答是不是更对、更有用要判官模型来判，
那一轮没跑；二、**降温治不了编造**。同一批问题里「南京有什么好吃的」答出过
「鸿门大酒楼」「老北京涮羊肉」，那是 139M 模型的事实性问题，跟采样无关，
温度调低只会让它稳定地编同一个。

`top_p 0.9` 和 `max_tokens 512` 仍然是上游默认值，没测过。
